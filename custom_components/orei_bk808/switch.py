...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry, async_add_entities):
    ...
    for output_num in range(1, 9):
        ...
        entities.append(ArcToggleSwitch(coordinator, output_num, output_name, "arc"))
        entities.append(MuteToggleSwitch(coordinator, output_num, output_name, "mute"))
...
class MatrixPowerSwitch(SwitchEntity):
    ...
    _attr_translation_key = "matrix_power"
    ...
    async def async_update(self):
        """Fetch latest status."""
        try:
            result = await self.coordinator.get_video_status()
            self._attr_is_on = result.get("power", False)
        except Exception as e:
            _LOGGER.error("Update failed: %s", e)

class ArcToggleSwitch(SwitchEntity):
    """ARC toggle for specific output."""
    ...
    def __init__(self, coordinator, output_num: int, output_name: str, toggle_type: str):
        ...
        self._attr_unique_id = f"{coordinator.host}_{output_name.lower()}_{toggle_type}"
        self._attr_translation_key = f"output_{output_num}_{toggle_type}"
...
class MuteToggleSwitch(ArcToggleSwitch):
    """Mute toggle for specific output."""
    pass
